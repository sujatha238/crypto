...
def mgf1(input_data, length, hash=hashlib.sha1):
  ...
  while (len(output) < length):
    print(" len(output) = ",len(output),"   length = ", length)
    print("input_data = ", hex(input_data))
    print("counter = ",counter)
    output += hash(int.to_bytes(input_data,24,byteorder='big')).hexdigest()
    print("output = ",output)
    input_data += 1
    counter += 1
    print("\n\n\n")
...
seed= 0xaafd12f659cae63489b479e5076ddec2f06cb58f00000000
