...
def gen_ecc_add(x1,z1,x2,z2,xp):
    #xp = 9
    #xp = (x2 - x1)%prime
    xp1 = ((x1*z2 - x2*z1) ** 2)%prime
    xp1 = pow(xp1,prime-2,prime)  #inverse
    xp =  (((x1*x2 - z1*z2) ** 2) * xp1)%prime
    print(" xp = ",hex(xp))
...
# From https://cr.yp.to/ecdh/curvezero-20060726.pdf
# X(Q-R) =   ((x' + x)(1+xx')  + 2axx'  + 2byy') / (x' - x)^2

inv_xd_x = (sx - rx)%prime

x_Q_R =  ((sx+rx)*(1+rx*sx))%prime
x_Q_R = x_Q_R + ((2*486662*rx*sx) + (2*ry*sy))%prime

x_Q_R = (x_Q_R * inv_xd_x * inv_xd_x) % prime
