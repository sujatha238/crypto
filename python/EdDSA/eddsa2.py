...
class PureEdDSA:
    ...
    def keygen(self,privkey):
        ...
        if(debug):
            ...
            print(" khash = SHA512(privkey) :  ", khash.hex())
        a=from_le(self.__clamp(khash[:self.b//8]))
        dbg_khash = khash[:self.b//8]
        if(debug):
            print("khash[0...31] :",khash[:self.b//8].hex())
            print("__clamp(dbg_khash) :",self.__clamp(dbg_khash).hex())
            print(" hex(a) = ", hex(a))
            dbg_ed_point = (self.B*a)
            #print(" type(dbg_ed_point) : ", type(dbg_ed_point))
            #print(" type(dbg_ed_point.xb) : ", Field.tobytes(dbg_ed_point.xb))
            #print(" dbg_ed_point : ", dbg_ed_point.encode())
            #print(" x of (self.B*a) : ", dbg_ed_point.xb.__x)
            #print(" y of (self.B*a) : ", dbg_ed_point.yb.__x)
            print(" (self.B*a).encode() : ", dbg_ed_point.encode().hex())
